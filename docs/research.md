...
Currently observed test results live in `[docs/testing.md](docs/testing.md)`. Hardware-specific expectations consumed by integration tests live as plists in `[Tests/IntegrationTests/Fixtures](Tests/IntegrationTests/Fixtures)`.
...
### Why does the tested M5 machine accept direct writes?

The tested M5 machine appears to accept direct writes to the lowercase mode key without the `Ftst` unlock. The reason is not clear from this research. A few non-exclusive possibilities are worth noting:
